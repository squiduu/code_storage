...
def prepare_data(args, tokenizer):
    if args["version"] == "2.0":
        paths = {k: f"ds2/data_mwoz_2.0/{k}_dials.json" for k in ("train", "dev", "test")}
        ontology = normalize_ontology(json.load(open("ds2/data_mwoz_2.0/mwz/ontology.json", "r")))
    else:
        paths = {k: f"ds2/data_mwoz_2.1/{k}_dials.json" for k in ("train", "dev", "test")}
    ...
    datasets = {
        k: DSTDataset(read_data(args, path, ALL_SLOTS, tokenizer, description, k), args) for k, path in paths.items()
    }
